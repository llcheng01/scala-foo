...
object BinaryTree {
  case class Node[T](
    ...
    override def isBalanced: Boolean = Math.abs(
      left.height - right.height
    ) <= 1 && left.isBalanced && right.isBalanced

    private def heightIterative: Int = {
      val res: Iterator[List[BinaryTree[T]]] = Iterator.iterate[List[BinaryTree[T]]](left :: right :: Nil) { levelItems =>
        levelItems
          .collect({ case Node(_, left, right) =>
            left :: right :: Nil
          }).flatten
      }

        res.takeWhile(_.nonEmpty)
        .size
    }

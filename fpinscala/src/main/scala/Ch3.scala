...
object Ch3 {
  ...
  object LinkedList {
    ...
    def tail[A](ll: LinkedList[A]): LinkedList[A] =
      ll match {
        case Nil =>
          throw new NoSuchElementException("cannot return tail of empty list")
    ...
    def dropWhile[A](ll: LinkedList[A], f: A => Boolean): LinkedList[A] = {
      ll match {
        ...
        case Cons(head, tail) if f(head) => {
          println("I am here"); dropWhile(tail, f)
        }
        case _ => ll
      }
    }
    ...
    def init[A](ll: LinkedList[A]): LinkedList[A] =
      ll match {
        case Nil =>
          throw new NoSuchElementException("cannot return init of empty list")
        case Cons(_, Nil)     => Nil
